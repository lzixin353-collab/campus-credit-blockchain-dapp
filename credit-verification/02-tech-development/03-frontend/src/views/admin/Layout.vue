  ...
  <el-container style="height: 100vh;">
    <el-aside width="200px" style="background-color: #2e3b4e;">
      <el-menu
        ...
        @select="onMenuSelect"
      >
        <el-menu-item index="/admin/credit-audit">
          <span>学分审核</span>
        </el-menu-item>
...
const shortAddress = computed(() => {
  const addr = address.value || userStore.address
  return addr ? `${String(addr).slice(0, 6)}...${String(addr).slice(-4)}` : ''
})

const onMenuSelect = (index) => {
  if (index === 'logout') logout()
